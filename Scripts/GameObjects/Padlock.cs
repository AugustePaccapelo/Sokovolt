...
namespace Com.IsartDigital.SokoVolt {
	...
	public partial class Padlock : Node2D
	{
		...
		[Export] private Node2D impactEffect;
		...
		public void Open()
		{
			...
			foreach (PointLight2D light in listLight) lTween.TweenProperty(light, "energy", 1, 1);
			lTween.SetParallel(false);
            lTween.TweenProperty(this, "modulate", new Color(0, 0, 0, 0), 0.5f);
            lTween.TweenProperty(impactEffect, "modulate", new Color(0, 0, 0, +1), 0.5f).AsRelative();
        }

        public void Close()
		{
            Tween lTween = CreateTween();
            lTween.TweenProperty(this, "modulate", new Color(3, 3, 3, 1), 0.5f).AsRelative();
            lTween.TweenProperty(impactEffect, "modulate", new Color(0, 0, 0, -1), 0.5f).AsRelative();
